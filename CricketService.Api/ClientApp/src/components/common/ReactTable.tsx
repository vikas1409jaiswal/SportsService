...
interface CustomTableInstance<T extends Record<string, unknown>> extends TableInstance<T> {
    page: any;
...
const IndeterminateCheckbox: React.FunctionComponent<IndeterminateCheckboxProps> = React.forwardRef(
    ({ indeterminate, ...rest }, reference) => {
...
export const ReactTable: React.FunctionComponent<ReactTableProps> = ({
    ...
    const {
        ...
        useRowSelect,
        hooks => {
            isRowSelect && hooks.visibleColumns.push(columns => [
                {
                    id: 'selection',
    ...
    $(document).ready(function () {
        $('.react-table-data-rows').each(function (index, element) {
            if (index === highlightedRowIndex) {
                $(element).css('background-color', highlightedRowCss.backgroundColor as string);
    ...
    return (
        <>
            <div className={`react-table ${className}`}>
                {
                    isSelectionPanel && <div className="react-table-selection-panel">
                        <div className="button-panel">
                            <button onClick={() => randomizeColumns()}>Randomize Columns</button>
                            <button onClick={resetResizing}>Reset Resizing</button>
                            <button onClick={() => toggleStateColumnFilter(!isEnableColumnFilter)}>
                ...
                <div className='react-table-container'>
                    <div className='table-preceder-panel'>
                        <p>{
                            isRowSelect ? <span>{`${Object.keys(selectedRowIds).length}`} rows selected.</span> : <span>Row selection disable.</span>
                            }
                            <button onClick={() => toggleStateColumnSetting(!isEnableColumnSetting)}>...</button>
                            <div className='column-setting-panel' style={{ visibility: isEnableColumnSetting ? 'visible' : 'hidden'} }>
                                {
                    ...
                    <table {...getTableProps()}>
                                    ...
                                    {headerGroup.headers.map((column: any) => (
                                        <th {...column.getHeaderProps(column.getSortByToggleProps())}>
                                            <>
                                                ...
                                                {
                                                    isEnableColumnFilter && <div className='column-filter'>
                                        ...
                                        <tr {...row.getRowProps()}
                                            ...
                                            {row.cells.map((cell: any) => {
                                                return <td {...cell.getCellProps()}
                                                    onClick={(e: any) => {
                                                        console.log(row)
                                                        cell.column.id !== 'selection' && handleRowClick(e, row)
                                                    }}
                                                >
                                                    {cell.render('Cell')}
                                                </td>
                        ...
                        <tfoot>
                            {isFooter && (footerGroups).map((footerGroup) => (
                ...
                {
                    isPagination && <div className='react-pagination-container'>
