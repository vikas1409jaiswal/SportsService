...
export const PlayerDetailsTable: React.FunctionComponent<
  PlayerDetailsTableProps
> = ({ playerData, isLoading, cricketFormat }) => {
  const results =
    cricketFormat === CricketFormat.ODI
      ? playerData?.careerDetails.odiCareer
      : playerData?.careerDetails.t20Career;
        ...
        {!isLoading && (
          <tbody>
            <tr>
              ...
              <td>
                <p>
                  {results?.debutDetails?.date} vs{" "}
                  {results?.debutDetails?.opponent}
                </p>
