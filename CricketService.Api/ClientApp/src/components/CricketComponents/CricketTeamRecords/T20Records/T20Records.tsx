...
import { NumberRangeColumnFilter, SliderColumnFilter } from '../../../common/Filter';
...
export const T20Records: React.FunctionComponent<T20RecordsProps> = ({ teamData, isLoading }) => {
    ...
    const columns = useMemo(() => [
        {
            ...
            Cell: (cell: Cell) =>
                <div>
                    <img
                        src={teamData.find(t => t.teamName === cell.row.values.teamName)?.flagUri}
        ...
        },
        {
            ...
            Footer: (info: any) => {
                const total = React.useMemo(
                    () =>
                        info.rows.reduce((sum: number, row: any) => row.values["teamRecordDetails.t20IResults.matches"] + sum, 0),
                    [info.rows]
                )
        ...
        },
        {
            ...
            Footer: (info: any) => {
                const total = React.useMemo(
                    () =>
                        info.rows.reduce((sum: number, row: any) => row.values["teamRecordDetails.t20IResults.won"] + sum, 0),
                    [info.rows]
                )
        ...
        },
        {
            ...
            Footer: (info: any) => {
                const total = React.useMemo(
                    () =>
                        info.rows.reduce((sum: number, row: any) => row.values["teamRecordDetails.t20IResults.lost"] + sum, 0),
                    [info.rows]
                )
        ...
        },
        {
            ...
            Footer: (info: any) => {
                const total = React.useMemo(
                    () =>
                        info.rows.reduce((sum: number, row: any) => row.values["teamRecordDetails.t20IResults.tiedAndWon"] + sum, 0),
                    [info.rows]
                )
        ...
        },
        {
            ...
            Footer: (info: any) => {
                const total = React.useMemo(
                    () =>
                        info.rows.reduce((sum: number, row: any) => row.values["teamRecordDetails.t20IResults.tiedAndLost"] + sum, 0),
                    [info.rows]
                )
        ...
        },
        {
            ...
            Footer: (info: any) => {
                const total = React.useMemo(
                    () =>
                        info.rows.reduce((sum: number, row: any) => row.values["teamRecordDetails.t20IResults.noResult"] + sum, 0),
                    [info.rows]
                )
    ...
    const filteredTeamData = teamData?.filter(x => x.teamRecordDetails.t20IResults !== null);
    const teamSelectedData = filteredTeamData?.find(x => x.teamUuid === selectedTeamUuid);
