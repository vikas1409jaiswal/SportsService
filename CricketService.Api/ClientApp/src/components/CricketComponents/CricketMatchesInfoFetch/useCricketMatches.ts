...
const fetchCricketMatches = (format: CricketFormat, year: number): Promise<AxiosResponse<ApiData>> => {
    let classNo = 0;
...
export const useTestCricketMatchesBySeasonDetails = (
    ...
    result.map((r, i) => {
        ...
        const tablesTeam1Data = team1Data.map(td => td?.querySelectorAll("table.ds-w-full"));
        const tablesTeam2Data = team2Data.map(td => td?.querySelectorAll("table.ds-w-full"));
                ...
                .forEach((tr) => {
                    ...
                    if (pName.includes("Fall of wickets:") && teamDetails) {
                        scoreSelector
                            ...
                            .forEach((x, i) => {
                                i === 0 &&
                                    teamDetails.inning1["fallOfWickets"].push(x?.textContent as string);
                                i > 0 &&
                ...
                .forEach((tr) => {
                    ...
                    if (pName.includes("Fall of wickets:") && teamDetails) {
                        scoreSelector
                            ...
                            .forEach((x, i) => {
                                i === 0 &&
                                    teamDetails.inning2["fallOfWickets"].push(x?.textContent as string);
                                i > 0 &&
