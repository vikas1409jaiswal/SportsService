...
export const SquadPlayerCard: React.FC<SquadPlayerCardProps> = ({
  ...
  useEffect(() => {
    index === 0 &&
      speakText(
        `${teamName} national cricket team squad for Sri Lanka tour of Zimbabwe ODI series`,
  ...
  useEffect(() => {
    inView &&
      speakText(
        `${player.name
          ?.replace("(c)", "captain")
          ?.replace("(vc)", "vice captain")}, ${player.role}, Age: ${player.age
          ?.split(" ")[0]
          ?.replace("y", "year")}, ${player.batting?.toLowerCase()} ${
          player.bowling ? "and " : ""
        }${player.bowling?.toLowerCase()}`,
        SpeechLanguage.HindiIndian,
        false
      );
  }, [inView, index]);
  ...
  return (
    <div
      ...
      style={{background: `linear-gradient(-15deg, ${teamLogos.find((logo) => logo.teamName.toLocaleLowerCase() === teamName?.replace("-women", "")?.toLocaleLowerCase())?.primaryColor},
      ${teamLogos.find((logo) => logo.teamName.toLocaleLowerCase() === teamName?.replace("-women", "")?.toLocaleLowerCase())?.secondaryColor})`}}
    >
      <img
        ...
        height={650}
        width={500}
        style={{maxWidth: 500, background: 'radial-gradient(white, rgb(231, 228, 11), black)', border: '3px solid white', borderRadius: 20}}
      />
