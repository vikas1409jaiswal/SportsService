...
const urls = [
  "https://www.espncricinfo.com/series/icc-women-s-world-cup-2025-26-1478193/india-women-squad-1499601/series-squads"]
              ...
              !x.isWithdrawn && (
                <div>
                  <img
                    ...
                    width={150}
                    alt={x.name}
                    style={{background: 'radial-gradient(white, rgb(231, 228, 11), black)', border: '3px solid white', borderRadius: 20}}
                  />
                  <h6>{x.name}</h6>
      ...
      {enableAnimation && (
        ...
        >
          <header className="team-squad-header">
            {`Sri Lanka tour of Zimbabwe ODI series- ${squadInfo?.teamName
              ?.toUpperCase()
              ?.replaceAll("-", " ")}`}
          </header>
          {selectedUrlIndex < 10 && (
            <div className="squad-players-list">
              <MovingTrain
                bogies={squadCards}
                trackLength={(12230 * 16) / 15 + 300 -100}
                duration={8.8 * 16}
                delay={12}
              />
            </div>
